...
namespace CircuitTool
{
    ...
    /// double bandwidth = OpAmpCalculator.ClosedLoopBandwidth(1e6, 100); // Closed-loop bandwidth
    /// </code>
    /// </remarks>
    public static class OpAmpCalculator
    {
        ...
        {
            // Differential gain uses the same formula as inverting gain: Rf / Rin
            return InvertingGain(feedbackResistor, inputResistor);
        }
        ...
        public static double SallenKeyCutoffFrequency(double r1, double r2, double c1, double c2)
        {
            ...
            return SecondOrderRCCutoffFrequency(r1, r2, c1, c2);
        }
        ...
        // Shared formula: fc = 1 / (2π √(R1·R2·C1·C2))
        private static double SecondOrderRCCutoffFrequency(double r1, double r2, double c1, double c2) =>
            1.0 / (2 * Math.PI * Math.Sqrt(r1 * r2 * c1 * c2));
