...
namespace CircuitTool
{
    ...
    public static class DigitalCalculator
    {
        ...
        public static double MinimumTraceWidth(double current, double temperatureRise, double copperThickness = 1.0)
        {
            ...
            // IPC-2221 formula: area in mils², then convert to m²
            const double milToMeter = 25.4e-6; // 1 mil = 25.4 µm = 25.4e-6 m
            double k = 0.024; // Constant for internal traces
            double areaMils2 = Math.Pow(current / (k * Math.Pow(temperatureRise, 0.44)), 1.0 / 0.725);
            double areaM2 = areaMils2 * milToMeter * milToMeter; // mils² → m²
            double thicknessM = copperThickness * 35e-6; // oz → m (1 oz ≈ 35 µm)
