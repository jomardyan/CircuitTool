...
namespace CircuitTool.Tests
{
    ...
    public class VoltageDividerCalculatorTests
    {
        ...
        [Test]
        public void Calculate_ZeroTotalResistance_ThrowsException()
        {
            Assert.Throws<ArgumentException>(() => VoltageDividerCalculator.Calculate(5.0, 0, 0));
        }

        [Test]
        public void Calculate_NegativeResistance_ThrowsException()
        {
            Assert.Throws<ArgumentException>(() => VoltageDividerCalculator.Calculate(5.0, -10, 100));
            Assert.Throws<ArgumentException>(() => VoltageDividerCalculator.Calculate(5.0, 10, -100));
        }
    }
}
